...
export function shootBullet(rotation) {
  ...
  const speedBonus = upgrade.bulletspeed;
    ...
    case 'pistol':
      if (weapon.ammo > 0) {
        ...
        for (let s = 0; s < shots; s++) {
          ...
          if (!bullet) break;
          bullet.fire(rotation + angleOffset(s), player.x, player.y, 4000 + speedBonus, 4500 + speedBonus, 0.02, Math.max(0.005, 0.04 + spreadMod), 100, false, damage);
        }
